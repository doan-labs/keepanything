...
const bearing = (a: number[], b: number[]) => {
  ...
  const x =
    Math.cos(a[1] * r) * Math.sin(b[1] * r) - Math.sin(a[1] * r) * Math.cos(b[1] * r) * Math.cos((b[0] - a[0]) * r)
...
export const CesiumFlythrough: React.FC<CesiumFlythroughProps> = ({
  ...
  const [handle] = useState(() => delayRender(`cesium init: ${mode}`, { timeoutInMilliseconds: 120000 }))
  const walker = useMemo(() => makePathWalker(smoothFlightPath(path, pathSmoothingPasses)), [path, pathSmoothingPasses])
  ...
  const setCamera = (C: any, viewer: any, progress: number) => {
    ...
    viewer.camera.setView({
      destination: C.Cartesian3.fromDegrees(camera[0], camera[1], lerp(altitudeStart, altitudeEnd, progress)),
      orientation: {
        heading,
    ...
    ;(async () => {
      ...
      if (mode === 'landscape') {
        ...
        viewer.creditDisplay.addStaticCredit(
          new C.Credit('<a href="https://www.maptiler.com/copyright/" target="_blank">&copy; MapTiler</a>', true)
        )
      }
      if (mode === 'city') {
