...
| Need | Use |
| --- | --- |
| Check a change in the running dev app (real dev library, hot reload) | `cdp.mjs` against `cd electron && pnpm run dev --remoteDebuggingPort 9222` |
| Layout of every card state, no library at all | the renderer in headless Chromium with the mock bridge |
| A clean, repeatable capture (seeded content, 1440x900, forced theme, native chrome) | `cd electron && pnpm run screenshot` |
| A check that should keep running | a Playwright `_electron` test in `electron/tests/e2e/` |
...
If it does not, ask the user to restart dev with `cd electron && pnpm run dev --remoteDebuggingPort 9222`
...
- `invoke` returns the `IpcEnvelope`: `{ ok: true, data }` or `{ ok: false, error }`. Payloads are
  ...
  in `electron/src/shared/ipc.ts` (`IpcRequestMap`).
...
- `shot` captures web contents only: no traffic lights, and no vibrancy behind the translucent
  sidebar. Use `cd electron && pnpm run screenshot` when the native chrome matters.
- Prefer `snap` to `shot` for structural checks: it is cheaper, and its names are what Playwright
  selects by (`getByRole('button', { name: 'Settings' })` in `electron/scripts/screenshot.mjs`).
...
With no preload bridge, the renderer falls back to `createMockBridge()` (`electron/src/renderer/src/lib/mock-fixtures.ts`):
...
```bash
cd electron && pnpm exec playwright screenshot --viewport-size=1440,900 --wait-for-timeout=1500 \
...
## Built app (`cd electron && pnpm run screenshot`)
...
```bash
cd electron && pnpm run screenshot                                # dark, seeded library
cd electron && pnpm run screenshot -- --theme light               # -> .artifacts/screenshot-light.png
cd electron && pnpm run screenshot -- --empty --reset             # empty state on a fresh profile
cd electron && pnpm run screenshot -- --palette "invoice"         # the Cmd+K palette with a query typed
cd electron && pnpm run screenshot -- --settings --settings-section "Provider"
cd electron && pnpm run screenshot -- --activity
```

It uses the `e2e` profile, so do not run it while `pnpm run test:e2e` (in `electron/`) is running.
