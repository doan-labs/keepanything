...
in case the primary provider goes down.

## Candidates

| Provider | What I tried | Notes |
| --- | --- | --- |
| GMI Cloud | `MiniMaxAI/MiniMax-M3` via the OpenAI-compatible `/v1/chat/completions` | Tool calling works. Vision works with `image_url` data URIs. `response_format: json_schema` is accepted but not enforced. Occasional 429 "all endpoints overloaded" with `retry-after: 60`. |
...
- Fallback: OpenRouter with the same model, same OpenAI-compatible transport.
...
Tags I would have used if this app needed tags: inference, pricing, gpu, llm-serving.
