...
- CORRECTION: the `swift: S-STORE-03 passes` commit was pushed while the gate was actually failing (a read-only open of the checkpointed fixture needs to create `-shm`, which SQLITE_OPEN_READONLY forbids — Electron only opens read-only while the lock holder has the db open, so `-shm` exists there). Fixed in `01f8c0e`: readOnly open tries a true read-only DatabasePool, falls back on SQLITE_CANTOPEN to a handle that refuses writes (`DbError.readOnly`) before touching sqlite. Gate re-run after the fix: Compat 3/3 (x3), KAStorageTests 45/45. REVIEW: the fallback is more permissive than Electron in this file state.

## 2026-09-26 S-CORE-01 passes
- `Sources/KACore/` (Ids, Errors/KaError, Ports: Logger/Clock/DomainEvent/EventBus/ItemPipeline, Events, Clock, Audit with real undo, ItemService, CollectionService, RelationshipService, statusForReprocess), `Sources/KAPipeline/{Graph,Queue}.swift` (graph.ts + queue.ts ported early — the services tests assert real job rows), `Sources/KATestSupport/Harness.swift` (harness.ts port minus StateApplier). KAModel gained public inits and the IPC request/event structs.
- Evidence: `swift test --filter KACoreTests` 23/23 (all 18 services its + bus/ids); KAStorageTests 45/45; lint ok (macOS only).
- REVIEW: TS `undefined`-vs-`null` patch semantics collapsed to Optional (`rename` has a `descriptionSet:` variant); `Audit.record`/`get` swallow repo errors with `try?` (TS propagates); afterCommit payload lookups also `try?` (unobservable in TS since events are fire-and-forget).
